...
<html lang="en">
  ...
  <body>
	  ...
	  <div class="container">
		  ...
		  <h3>Levels (Plans)</h3>
		  <?php
		  $results = mysqli_query($conn, "SELECT * FROM levels");
				echo "<table class=\"table table-striped\">";
			  echo "
				  <thead>
				  <tr>
					<th>ID</th>
					<th>RAM balance (MB)</th>
					<th>Max disk (MB)</th>
					<th>Max servers</th>
					<th>Max sites</th>
					<th>Max cores</th>
			  ...
			  if( $results->num_rows !== 0 ) {
				 while($rowitem = mysqli_fetch_array($results)) {
					echo "<tr>";
					echo "<td>" . $rowitem['level'] . "</td>";
					echo "<td>" . $rowitem['ram_balance'] . "</td>";
					echo "<td>" . $rowitem['max_disk'] . "</td>";
					echo "<td>" . $rowitem['max_servers'] . "</td>";
					echo "<td>" . $rowitem['max_sites'] . "</td>";
					echo "<td>" . $rowitem['max_cores'] . "</td>";
					echo "<td>" . $rowitem['title'] . "</td>";
					echo "<td>" . $rowitem['price'] . "</td>";
					if( $rowitem['ismonthly'] == 1 ) {
						$ismonthly = "Yes";
					} else {
						$ismonthly = "No";
					}
					echo "<td>" . $ismonthly . "</td>";
					echo "</tr>";
				}
			  } else {
				  echo "There's no levels to show.";
			  }
