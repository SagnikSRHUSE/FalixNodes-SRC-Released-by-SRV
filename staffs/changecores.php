...
if( !isset($_GET['id']) || empty($_GET['id']) || !isset($_GET['newcores']) || empty($_GET['newcores']) ) {
	header("Location: /");
	die();
}

if(is_numeric($_GET['newcores']) && $_GET['newcores'] > 0 && $_GET['newcores'] == round($_GET['newcores'], 0)){
	//pass
} else {
	ShowError("Cores must be a number.");
}

// Get some server information, those are needed to update cores
...
$log_message = "Changed cores of server #" . $_GET['id'] . " owned by " . $server_owner_id . " to " . $_GET['newcores'];
$log_time = time();
